...
## معايير الأداء المحلية — Apple Silicon (MLX، 8-bit)

أرقام H200 السحابية أدناه تخص وحدة معالجة رسومات في مركز بيانات. إليك كيف يعمل **إصدار 8-bit MLX** فعليًا على جهاز Mac عادي بمعالج Apple Silicon — تم القياس على خادم `local_mlx` بعد تسخينه (`:8881`)، مع تخصيص وحدة معالجة الرسومات بالكامل له.

| المقطع | الصوت الناتج | زمن التوليد | RTF (التوليد ÷ الصوت) | رموز دلالية/ث |
|--------|--------------|-------------|------------------------|----------------|
| قصير  | 4.55 s  | 17.0 s | 3.74 | 5.8 |
| متوسط | 7.66 s  | 28.4 s | 3.70 | 5.8 |
| طويل  | 11.89 s | 45.4 s | 3.82 | 5.6 |
| **الإجمالي** | **24.1 s** | **90.7 s** | **3.77** | **~5.7** |

يعمل بمعدل **RTF يقارب 3.8** (نحو 0.27× الزمن الفعلي) — أداة عمل للمعالجة دون اتصال، وليست محرك بث منخفض زمن الاستجابة. عنق الزجاجة هو توليد الرموز الدلالية (نحو 5.7 رمز/ث) وليس المُرمِّز (codec). يستغرق البدء البارد نحو 23 ثانية لتحميل الأوزان، دون أي تكلفة تحميل لكل طلب بعد ذلك.

تم التحقق من كل مقطع عبر ASR على `:5093` (نُسخ بشكل صحيح، بذُرى 0.53–0.93). استخدم **8-bit فقط**: تحويلات 4-bit في MLX تُفكَّك إلى ضوضاء (هذا هو التكميم التآلفي لـ Apple Silicon، ولا علاقة له بإصدار CUDA `bnb4` NF4 في هذا الـ fork). الدليل الكامل: [`local_mlx/README.md`](../local_mlx/README.md).
