...
## 로컬 벤치마크 — Apple Silicon (MLX, 8-bit)

아래의 H200 클라우드 수치는 데이터센터 GPU 기준입니다. 여기서는 **8-bit MLX 빌드** 가 일반적인 Apple Silicon Mac에서 실제로 어떻게 동작하는지 보여줍니다 — 워밍업된 `local_mlx` 서버(`:8881`)에서 GPU를 독점한 상태로 측정했습니다.

| 클립 | 출력 오디오 | 생성 시간 | RTF (생성 ÷ 오디오) | 시맨틱 tok/s |
|------|-------------|-----------|---------------------|--------------|
| 짧음 | 4.55 s  | 17.0 s | 3.74 | 5.8 |
| 중간 | 7.66 s  | 28.4 s | 3.70 | 5.8 |
| 김   | 11.89 s | 45.4 s | 3.82 | 5.6 |
| **합계** | **24.1 s** | **90.7 s** | **3.77** | **~5.7** |

측정 결과 **RTF 약 3.8**(약 0.27× 실시간)로, 저지연 스트리밍 엔진이 아니라 오프라인 배치용 작업 도구에 적합합니다. 병목은 시맨틱 토큰 생성(약 5.7 tok/s)이며 코덱이 아닙니다. 콜드 스타트는 가중치 로드에 약 23초가 걸리며, 이후에는 요청마다 로드 비용이 없습니다.

모든 클립은 `:5093`의 ASR로 검증했습니다(정확히 전사됨, 피크 0.53–0.93). **8-bit만 사용하세요**: MLX의 4-bit 변환은 노이즈로 디코딩됩니다(이는 Apple Silicon의 어파인 양자화로, 이 fork의 CUDA `bnb4` NF4 빌드와는 무관합니다). 전체 가이드: [`local_mlx/README.md`](../local_mlx/README.md).
