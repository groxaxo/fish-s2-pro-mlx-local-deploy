...
## ローカルベンチマーク — Apple Silicon（MLX、8-bit）

下記の H200 クラウドの数値はデータセンター GPU のものです。ここでは **8-bit MLX ビルド** が一般的な Apple Silicon Mac で実際にどう動くかを示します —— ウォームアップ済みの `local_mlx` サーバー（`:8881`）で、GPU を専有して測定しました。

| クリップ | 出力音声 | 生成時間 | RTF（生成 ÷ 音声） | セマンティック tok/秒 |
|----------|----------|----------|--------------------|----------------------|
| 短   | 4.55 s  | 17.0 s | 3.74 | 5.8 |
| 中   | 7.66 s  | 28.4 s | 3.70 | 5.8 |
| 長   | 11.89 s | 45.4 s | 3.82 | 5.6 |
| **合計** | **24.1 s** | **90.7 s** | **3.77** | **~5.7** |

実測で **RTF は約 3.8**（約 0.27× リアルタイム）—— 低遅延ストリーミング用ではなく、オフラインバッチ向けの実用機です。ボトルネックはセマンティックトークン生成（約 5.7 tok/s）であり、コーデックではありません。コールドスタートは重みのロードに約 23 秒かかり、その後はリクエストごとのロードコストはありません。

各クリップは `:5093` の ASR で検証済みです（正しく文字起こしされ、ピークは 0.53–0.93）。**8-bit のみを使用してください**：MLX の 4-bit 変換はノイズにデコードされます（これは Apple Silicon のアフィン量子化であり、本 fork の CUDA `bnb4` NF4 ビルドとは無関係です）。詳細は [`local_mlx/README.md`](../local_mlx/README.md) を参照してください。
