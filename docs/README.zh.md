...
## 本地基准测试 — Apple Silicon（MLX，8-bit）

下方的 H200 云端数据来自数据中心 GPU。这里是 **8-bit MLX 版本** 在普通 Apple Silicon Mac 上的实际表现 —— 在已预热的 `local_mlx` 服务（`:8881`）上测得，GPU 独占。

| 片段 | 输出音频 | 生成耗时 | RTF（生成 ÷ 音频） | 语义 token/秒 |
|------|----------|----------|--------------------|---------------|
| 短   | 4.55 s  | 17.0 s | 3.74 | 5.8 |
| 中   | 7.66 s  | 28.4 s | 3.70 | 5.8 |
| 长   | 11.89 s | 45.4 s | 3.82 | 5.6 |
| **合计** | **24.1 s** | **90.7 s** | **3.77** | **~5.7** |

实测 **RTF 约为 3.8**（约 0.27× 实时）—— 适合离线批量合成，而非低延迟流式场景。瓶颈在于语义 token 的生成（约 5.7 tok/s），而非编解码器。冷启动加载权重约需 23 秒，之后每次请求不再有额外加载开销。

每个片段都通过 `:5093` 上的 ASR 进行了验证（转写正确，峰值 0.53–0.93）。**请仅使用 8-bit**：MLX 的 4-bit 转换会解码为噪声（这是 Apple Silicon 的仿射量化，与本 fork 的 CUDA `bnb4` NF4 版本无关）。完整指南见 [`local_mlx/README.md`](../local_mlx/README.md)。
